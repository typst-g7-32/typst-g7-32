...
- entry_id (integer, primary key) – уникальный идентификатор записи.
- name_id (integer, foreign key, not null) – внешний ключ к таблице names.
- surname_id (integer, foreign key, not null) – внешний ключ к таблице surnames.
- patronymic_id (integer, foreign key, not null) – внешний ключ к таблице patronymics.
- street_id (integer, foreign key, not null) – внешний ключ к таблице streets.
...
Интерфейс родительской таблицы показан на рисунке @parent_table.
...
Контекстное меню программы, позволяющее работать с несколькими строками, приведено на рисунке @context_menu.
...
Логирование осуществляется с помощью библиотеки loguru, что позволяет отслеживать выполнение программы и фиксировать SQL‑запросы.
...
Параметры подключения к базе данных задаются через файл .env. Пример содержимого файла конфигурации показан на рисунке @dotenv.
...
+ Сброс базы данных
   ...
   Функция сброса базы данных удаляет все записи и обнуляет счётчики PostgreSQL. Диалоговое окно сброса показано на рисунке @reset_db_dialog.
